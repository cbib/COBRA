...
if "log" not in globals():
  logger = Logger.init_logger('PROCESS_QTLS_%s'%(cfg.language_code), load_config())
...
qtls_to_process=qtls_col.find({"mapping_file":{"$exists":False}})

logger.info("Found %d qtl tables to process",qtls_to_process.count())

for map_doc in qtls_to_process:
	...
	try:
                qtls_col.update({"_id":map_doc['_id']},{"$set":{"mapping_file":sheet_values}})
		#break
		...
		qtls_col.update({"_id":map_doc['_id']},{"$set":{"mapping_file":{"species":species,"file":fp.data_file}}})
		# Close opened file
		file.close()
...
logger.info("Indexation on field \"mapping_file.QTL ID\" from collection \"QTLS\"")
qtls_col.create_index("mapping_file.QTL ID",sparse=True)