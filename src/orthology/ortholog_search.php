    ...
    if ($species=="Prunus species"){
        ...
        foreach ($species_id_type as $species_value) {
            ...
            foreach ($gene_list_attributes as $attributes) {
                ...
                foreach ($attributes as $key => $value) {
                    if ($cpt==0){
                        echo '<div style="cursor: pointer;" onclick="window.open(https://services.cbib.u-bordeaux2.fr/cobra/src/result_search_5.php?organism='.str_replace(" ", "+", $species).'&search='.$attributes['search'].',\'_newtab\'); class="resultsbox" id="results">
