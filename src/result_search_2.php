...
//        }
    ...
    */
    ...
    $timestart=microtime(true);
    $srccol=$mappingsCollection->find(array('type'=>'gene_to_prot','species'=>$organism),array('src'=>1,'tgt'=>1));
    foreach ($srccol as $values) {
        echo 'gene_to_prot source type : '.$values['src'];
        echo '<hr>';
    ...
    $srccol=$mappingsCollection->find(array('type'=>'gene_to_symbol','species'=>$organism),array('src'=>1,'tgt'=>1));
    foreach ($srccol as $values) {
        echo 'gene_to_symbol source type : '.$values['src'];
        echo '<hr>';
        $cursor=$mappingsCollection->aggregate(array( 
            ...
            array('$unwind'=>'$mapping_file'),    
            array('$match' => array('mapping_file.'.$values['src']=>$search)),  
            array('$project' => array('tgt_id'=>'$mapping_file.'.$values['tgt'],'_id'=>0))
        ));
        //var_dump($cursor);
        foreach ($cursor['result'] as $result) {
            echo 'tgt id : '.$result['tgt_id'];
    ...
    $srccol=$mappingsCollection->find(array('type'=>'gene_to_gene','species'=>$organism),array('src'=>1,'tgt'=>1));
    foreach ($srccol as $values) {
        echo 'gene_to_gene source type : '.$values['src'];
        echo '<hr>';
        $cursor=$mappingsCollection->aggregate(array( 
    ...
    foreach ($srccol as $values) {
        echo 'gene_to_go source type : '.$values['src'];
        echo '<hr>';
    ...
    foreach ($tgtcol as $values) {
        echo 'est_to_gene source type : '.$values['tgt'];
        echo '<hr>';
        $cursor=$mappingsCollection->aggregate(array( 
   ...
   /* foreach ($mappings_to_process as $map_doc){
       ...
       // Old way to capture gene ontology
        ...
        //read the gene_to_go grid mapping file for this species or all species if the option is selected
        ...
        if ($type=='gene_to_go'){
            ...
            foreach ($map_file as $key=>$value){
                ...
                if($key=='file'){
                    if ($src_col=='plaza_gene_id'){
                        $go_grid_plaza_id_list=read_grid_plaza_mapping_file($grid, $mappingsCollection,$value,$plaza_id);
            ...
            echo "Debut du script: ".date("H:i:s", $timestart);
            echo "<br>Fin du script: ".date("H:i:s", $timeend);
            echo "<br>Script reading go in grid files executed in " . $page_load_time . " sec";
...
//                                       <dd>'.$value.'</dd>';
            ...
            echo "Debut du script: ".date("H:i:s", $timestart);
            echo "<br>Fin du script: ".date("H:i:s", $timeend);
            echo "<br>Script reading go in mapping document executed in " . $page_load_time . " sec";
        }
...
/*//	if ($type=='gene_to_prot'){
    ...
    //	
    //	}*/
/*new way for searching
//search for protein information
//    $cursor=$mappingsCollection->aggregate(array( 
//        array('$match' => array('type'=>'gene_to_prot')),  
//        array('$project' => array('tgt_to_src'=>1,'species'=>1,'_id'=>0)),    
//        array('$unwind'=>'$tgt_to_src'),    
//        array('$match' => array('src_to_tgt.0'=>$search)),  
//        array('$project' => array('src_to_tgt'=>1,'_id'=>0))
//    ));
//    foreach ($cursor as $key => $value) {
//        //echo 'tgt'.$value[0];
//        var_dump($value);
...
//        echo "Debut du script: ".date("H:i:s", $timestart);
//        echo "<br>Fin du script: ".date("H:i:s", $timeend);
//        echo "<br>Script reading grid file execute en " . $page_load_time . " sec";
//
//        echo '<hr>';
//        
//
...
//    $page_load_time1 = number_format($time, 3);
//    echo "Debut du script: ".date("H:i:s", $timestart1);
//    echo "<br>Fin du script: ".date("H:i:s", $timeend1);
//    echo "<br>Script comparing with all mapping executed in " . $page_load_time1 . " sec";
//    echo '<hr>';*/
...
//        echo "Debut du script: ".date("H:i:s", $timestart1);
//        echo "<br>Fin du script: ".date("H:i:s", $timeend1);
//        echo "<br>Script filling GO ID list execute en " . $page_load_time1 . " sec";
//
//        echo '<hr>';
    }
    //Categorize term from plaza GO annotation
    //$go_id='';
    //error_log('size : '.count($go_grid_plaza_id_list));
...
if (count($go_id_list)!=0){
    ...
    foreach ($go_id_list as $go_info){
        ...
        echo "Debut du script: ".date("H:i:s", $timestart1);
        echo "<br>Fin du script: ".date("H:i:s", $timeend1);
        echo "<br>Script foreach GO ID executed in " . $page_load_time1 . " sec";
...
echo "Debut du script: ".date("H:i:s", $timestart);
echo "<br>Fin du script: ".date("H:i:s", $timeend);
echo "<br>Script filling GO ID list execute en " . $page_load_time . " sec";
...
//            </div>    
//            <br/>*/
//old code for presenting GO results
